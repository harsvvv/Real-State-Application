...
export default function CreateListing() {
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto">
      <h1 className="text-3xl font-semibold my-7 text-center">CreateListing</h1>
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="flex flex-wrap gap-6">
            <div className="flex items-center gap-2">
                <input className="p-3 border border-gray-300 rounded-lg" type="number" id="bedrooms" min='1' max='10' onChange={handleChange}  checked={formData.bedrooms} required/>
                <p>Beds</p>
            </div>
            <div className="flex items-center gap-2">
                <input className="p-3 border border-gray-300 rounded-lg" type="number" id="bathrooms" min='1' max='10' onChange={handleChange}  checked={formData.bathrooms} required/>
                <p>Baths</p>
            </div>
            <div className="flex items-center gap-2">
                <input className="p-3 border border-gray-300 rounded-lg" type="number" id="regularPrice" onChange={handleChange}  checked={formData.regularPrice} min='50' max='100000000' required/>
            ...
            <div className="flex items-center gap-2">
                <input onChange={handleChange}  checked={formData.discountPrice} className="p-3 border border-gray-300 rounded-lg" type="number" id="discountPrice" min='0' max='10000000' required/>
                <div className="flex flex-col items-center">
                <p>Discount Price</p>
                <span className="text-sm">($/ months)</span>
                </div>
    ...
    formData.imageUrls.length > 0 && formData.imageUrls.map((object,index) => (
        ...
        <button disabled={loading || uploading} className="p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95">{loading?'loading...':'create Listing'}</button>
